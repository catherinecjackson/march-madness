...
for i = 1:n_runs
    ...
    if df[idx3, :ExitRound] > "4"
        totallikelihood = totallikelihood + 1.0
    end
    ...
    if df[idx6, :ExitRound] > "3"
        totallikelihood = totallikelihood + 0.50
    end
    ...
    totallikelihood = totallikelihood * 100;
    roundtotallikelihood = round(totallikelihood)
    if roundtotallikelihood > 28
        results[i, :Bracket] = selectedbracket
...
# Find the indices where the likelihood is the highest
max_likelihood_indices = findall(results.TotalLikelihood .== max_likelihood)

    # Display the brackets with the highest likelihood
